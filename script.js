...
selectTag.forEach((tag, id) => {
    for (const countryCodes in countries ) {
        let selected;
        if (id == 0 && countryCodes == 'en-GB'){
            selected = 'selected'
        }else if(id == 1 && countryCodes == 'uz-UZ'){
            selected = 'selected'
        }

        let option = `<option value="${countryCodes}" ${selected}>${countries[countryCodes]}</option>`
